...
import {
	ChatMessage,
	ConversationItem,
} from '../../interface/chatInterface.ts';
...
const ChatApp: React.FC = () => {
	...
	const [latestCreatedConversationId, setLatestCreatedConversationId] =
	...
	const [chatHistory, setChatHistory] = useState<ChatMessage[]>([
		...
		},
		{
			id: '3',
			content:
				'使用React开发聊天应用是个不错的选择！我可以为你提供一些基本的开发思路和组件建议。\n\n首先，你需要考虑以下几个方面：\n1. 消息组件设计\n2. 聊天界面布局\n3. 状态管理\n4. 数据持久化\n\n你想了解哪方面的具体内容呢？',
			role: 'assistant',
			timestamp: '10:32',
		},
	]);
			...
			if (selectedConversationId === conversationId) {
				...
				const newSelectedId =
					conversationList.length > 1
						? conversationList.find(item => item.id !== conversationId)?.id
						: '';
