...
export default tseslint.config(
	...
	{
		// extends配置：继承推荐的规则集
		// 包括ESLint推荐的JavaScript规则、TypeScript-ESLint推荐的TypeScript规则
		// 以及Prettier的配置，用于禁用与Prettier冲突的ESLint规则
		extends: [
		...
		plugins: {
			...
			// 启用Prettier插件，用于将Prettier作为ESLint规则运行
			prettier: prettier,
		},
		...
		rules: {
			...
			// 配置Prettier规则
			// 'error'级别表示违反Prettier格式时显示错误
			// 'singleQuote': true 表示使用单引号
			// 'trailingComma': 'es5' 表示在ES5语法中使用尾随逗号
			// 'printWidth': 80 表示每行最大宽度为80个字符
			// 'useTabs': true 表示使用tabs进行缩进，与项目ESLint配置保持一致
			'prettier/prettier': [
				'error',
				{
					singleQuote: true,
					...
					printWidth: 80,
					useTabs: true,
					endOfLine: 'auto', // 自动根据操作系统选择换行符格式
				},
			],
